...
//          dd($last_bill);
    ...
    public function uploadBill()
    {
        if (session('allocationno') && session('allocationno') != null) {
            ...
            $currentSupply = Bill::whereDate('created_at', '=', date('Y-m-d'))->get();
            ...
            return view('Users.Manager.allocation', compact('staff', 'currentSupply','UnallocatedBills','pastallocationBills'));
            }
            else {
...
//                  dd($data->toArray());
    ...
    public function searchAllocation(Request $request)
    {
        ...
        $data = Bill::where('allocationNo', $request->allocation)->with('field')->get()->toArray();
        dd($data);
        if ($data) {
